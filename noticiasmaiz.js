(function () {
  ...
  function downloadPdf() {
    ...
    const rowsHtml = state.filtered.map((it, i) => {
      ...
      return `
        <tr>
          <td style="padding:6px 8px;vertical-align:top;color:#6b7280;">${i + 1}.</td>
          <td style="padding:6px 8px;vertical-align:top;">
            <div style="font-weight:600;">${safeTitle}</div>
            <div style="font-size:12px;color:#374151;">${safeSrc}${dateStr ? ' • ' + dateStr : ''}</div>
            <div style="font-size:12px;color:#2563eb;word-break:break-all;">${safeLink}</div>
          </td>
        </tr>`;
    }).join('');
    ...
    <body>
      <h1>${title}</h1>
      <div class="meta">Generado: ${now} • Total: ${total}</div>
